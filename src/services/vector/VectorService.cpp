...
namespace services
{
    namespace vector
    {
        ...
        http::HttpResponse VectorService::UpsertPoint(const std::string &collection_name, const repositories::vector::VectorPoint &point) const
        {
            return vector_repository.UpsertPoint(collection_name, point);
        }

        http::HttpResponse VectorService::UpsertPoints(const std::string &collection_name, const std::vector<repositories::vector::VectorPoint> &points) const
        {
